...
void tcp_cb_init(struct iphdr *ip, struct tcphdr *tcp, struct tcp_cb *cb)
{
	...
	log(DEBUG, "tcp_cb_init: saddr %u.", cb->saddr);
	cb->daddr = ntohl(ip->daddr);
	log(DEBUG, "tcp_cb_init: daddr %u.", cb->daddr);
	cb->sport = ntohs(tcp->sport);
	log(DEBUG, "tcp_cb_init: sport %hu.", cb->sport);
	cb->dport = ntohs(tcp->dport);
	log(DEBUG, "tcp_cb_init: dport %hu.", cb->dport);
...
void handle_tcp_packet(char *packet, struct iphdr *ip, struct tcphdr *tcp)
{
	...
	if (!tsk) 
		log(ERROR, "handle_tcp_packet: tcp_sock_lookup failed to look a fit sock.");
