...
#define max(x,y) ((x)>(y) ? (x) : (y))	//modified
...
{
	log(DEBUG, "tcp_sock_set_sport: will to bind to port %hu ", port);
	...
	log(DEBUG, "tcp_sock_set_sport: bind to port %hu ", port);
	tcp_bind_hash(tsk);
...
int tcp_hash(struct tcp_sock *tsk)
{
	...
	if (tsk->state == TCP_LISTEN) {
		...
		log(DEBUG, "tcp_hash: hash to listen list with port %hu.", tsk->sk_sport);
	}
	else {
		...
		log(DEBUG, "tcp_hash: hash to established list with port %hu.", tsk->sk_sport);
	}
...
int tcp_sock_bind(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	tsk->sk_sip = ntohl(skaddr->ip);			//modified
...
//  fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    ...
    if (!list_empty(&tsk->accept_queue))
        ...
        if (sleep_on(tsk->wait_accept) < 0)
			log(ERROR, "tcp_sock_accept:fail to sleep on wait_accept.");
        csk = tcp_sock_accept_dequeue(tsk);
    }
...
{
//  fprintf(stdout, "TODO: implement %s please.\n", __FUNCTION__);
    int rdy_snd_len = 0;    //already send length
    ...
    while (rdy_snd_len < size) 
	{
        ...
        memcpy(packet + hdr_size, buf + rdy_snd_len, seg_size);

        tcp_send_packet(tsk, packet, pkt_size);
        rdy_snd_len += seg_size;
    }
